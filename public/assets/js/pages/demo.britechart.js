...
(function ($, context) {
  ...
  context.createBarChart = function (elementSelector, data) {
    ...
    var barContainer = d3.select(elementSelector),
        ...
        containerHeight = barContainer.node() ? barContainer.node().getBoundingClientRect().height : false,
    ...
    _barChart.isAnimated(true).width(containerWidth).height(containerHeight).margin(margin).betweenBarsPadding(0.5).colorSchema(colors).on("customMouseOver", chartTooltip.show).on("customMouseMove", chartTooltip.update).on("customMouseOut", chartTooltip.hide);
  ...
  context.createHorizontalBarChart = function (elementSelector, data) {
    ...
    _barChart.colorSchema(colors).isAnimated(true).isHorizontal(true).width(containerWidth).margin(margin).enableLabels(true).percentageAxisToMaxRatio(1.3).labelsNumberFormat(1).height(containerHeight);
  ...
  context.createLineChart = function (elementSelector, data) {
    var lineChart = new britecharts.line(),
        ...
        tooltipContainer = void 0;
    lineChart.isAnimated(true).aspectRatio(0.7).tooltipThreshold(100).grid("horizontal").width(containerWidth).dateLabel("date").valueLabel("value").on("customMouseOver", chartTooltip.show).on("customMouseMove", chartTooltip.update).on("customMouseOut", chartTooltip.hide);
    chartTooltip.title("Sample Data"); //   .topicsOrder(lineData2.dataByTopic.map(function(topic) {
    //   return topic.topic;
    // }));

    lineContainer.datum(data).call(lineChart);
    tooltipContainer = d3.select(elementSelector + " .metadata-group .hover-marker");
    tooltipContainer.datum([]).call(chartTooltip);
  };
  /**
  ...
  context.createDonutChart = function (elementSelector, data) {
    ...
    legendChart.width(250).height(200).colorSchema(colors).numberFormat("s");
    donutChart.height(300).highlightSliceById(3).colorSchema(colors).hasFixedHighlightedSlice(true).internalRadius(80).on('customMouseOver', function (data) {
      legendChart.highlight(data.data.id);
    }).on('customMouseOut', function () {
      legendChart.clearHighlight();
    });
    d3.select(elementSelector).datum(data).call(donutChart);
  ...
  context.createBrushChart = function (elementSelector, data) {
    ...
    brushChart.height(320).width(containerWidth).on("customBrushStart", function (brushExtent) {
  ...
  context.createStepChart = function (elementSelector, data) {
    var stepChart = britecharts.step(),
        ...
        tooltipContainer = void 0;
    stepChart.width(containerWidth).height(320).margin({
      top: 40,
      right: 40,
      bottom: 80,
      left: 50
    }).on("customMouseOver", chartTooltip.show).on("customMouseMove", chartTooltip.update).on("customMouseOut", chartTooltip.hide);
    stepContainer.datum(data).call(stepChart);
    chartTooltip.nameLabel("key");
    tooltipContainer = d3.select(elementSelector + " .step-chart .metadata-group");
    tooltipContainer.datum([]).call(chartTooltip);
  }; //private method
  ...
  function init() {
    ...
    function drawCharts() {
      ...
      if ($(".bar-container-horizontal").length > 0) {
        briteChartApp.createHorizontalBarChart(".bar-container-horizontal", barData);
      }
